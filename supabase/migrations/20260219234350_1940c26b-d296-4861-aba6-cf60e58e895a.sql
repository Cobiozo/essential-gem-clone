...
CREATE OR REPLACE FUNCTION public.get_leader_team_training_progress(p_leader_user_id uuid)
 RETURNS TABLE(user_id uuid, first_name text, last_name text, eq_id text, role text, module_id uuid, module_title text, assigned_at timestamp with time zone, is_completed boolean, total_lessons bigint, completed_lessons bigint, progress_percentage numeric, level integer)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    AND lp.can_view_team_progress = true
  ) THEN
  ...
  SELECT p.eq_id INTO v_leader_eq_id 
  FROM public.profiles p
  WHERE p.user_id = p_leader_user_id;
