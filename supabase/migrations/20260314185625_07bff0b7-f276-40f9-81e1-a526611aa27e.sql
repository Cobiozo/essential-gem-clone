...
-- 1. Create mfa_enforced_users table
CREATE TABLE public.mfa_enforced_users (
...
ALTER TABLE public.mfa_enforced_users ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Admins can view MFA enforcements"
  ON public.mfa_enforced_users FOR SELECT
  TO authenticated
  USING (public.has_role(auth.uid(), 'admin'));

CREATE POLICY "Admins can insert MFA enforcements"
  ON public.mfa_enforced_users FOR INSERT
  TO authenticated
  WITH CHECK (public.has_role(auth.uid(), 'admin'));

CREATE POLICY "Admins can delete MFA enforcements"
  ON public.mfa_enforced_users FOR DELETE
  TO authenticated
  USING (public.has_role(auth.uid(), 'admin'));

-- 2. Update get_my_mfa_config to check enforcements
...
BEGIN
  ...
  -- Check if user has individually enforced MFA
  IF EXISTS (SELECT 1 FROM public.mfa_enforced_users WHERE user_id = auth.uid()) THEN
    SELECT 
      CASE 
