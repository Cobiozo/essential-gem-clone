...
async function sendSmtpEmail(
  ...
  console.log(`[SMTP] Sending training reminder to ${to}`);
  ...
  try {
    ...
    console.log('[SMTP] Training reminder sent successfully');
    return { success: true };
...
serve(async (req) => {
  ...
  try {
    ...
    const { userId, moduleId, daysInactive, progressPercent, assignmentId }: TrainingReminderRequest = await req.json();
    console.log("[send-training-reminder] Request:", { userId, moduleId, daysInactive, progressPercent, assignmentId });
    ...
    const { data: templateData, error: templateError } = await supabase
      ...
      .select("*")
      .eq("internal_name", "training_reminder")
      .eq("is_active", true)
      .single();

    if (templateError || !templateData) {
      throw new Error("Email template 'training_reminder' not found");
    }
    ...
    const variables: Record<string, string> = {
      ...
      progress_percent: String(progressPercent || 0),
    ...
    // Update reminder tracking in training_assignments
    ...
    console.log("[send-training-reminder] Email sent successfully to:", profile.email);

    return new Response(
      JSON.stringify({ 
        success: true, 
        message: `Training reminder sent to ${profile.email}`,
    ...
    console.error("[send-training-reminder] Error:", error);
    return new Response(
