...
async function sendSmtpEmail(
  ...
  console.log(`[SMTP] Sending final inactivity warning to ${to}`);
  ...
  try {
    ...
    console.log('[SMTP] Final inactivity warning sent successfully');
    return { success: true };
...
serve(async (req) => {
  ...
  try {
    ...
    const { userId, daysInactive }: FinalWarningRequest = await req.json();
    console.log("[send-inactivity-final-warning] Request:", { userId, daysInactive });
    ...
    const htmlBody = `
      ...
      <div style="background:#F8D7DA;border:1px solid #F5C6CB;border-radius:8px;padding:16px;margin:20px 0;">
        <p style="color:#721C24;margin:0;font-weight:bold;font-size:16px;">🚨 PILNE — Twoje konto zostanie zablokowane za 24 godziny!</p>
        <p style="color:#721C24;margin:8px 0 0 0;">
          Od <strong>${daysInactive} dni</strong> nie logowałeś/aś się na platformie Pure Life Center.
          Jutro o tej porze Twoje konto zostanie <strong>automatycznie zablokowane</strong>.
        </p>
      </div>
      <p style="color:#555;line-height:1.6;">
        Aby <strong>zapobiec blokadzie</strong>, wystarczy zalogować się na platformę:
      </p>
      <div style="text-align:center;margin:25px 0;">
        <a href="${baseUrl}" style="background:linear-gradient(135deg,#DC3545,#C82333);color:#fff;padding:14px 32px;border-radius:8px;text-decoration:none;font-weight:bold;font-size:16px;display:inline-block;">
          🔑 Zaloguj się teraz — masz 24h!
        </a>
      </div>
      <div style="background:#f0f0f0;border-radius:8px;padding:16px;margin:20px 0;">
        <p style="color:#666;margin:0;font-size:14px;">
          <strong>Co się stanie po zablokowaniu konta?</strong>
        </p>
        <ul style="color:#666;font-size:14px;margin:8px 0 0 0;padding-left:20px;">
          <li>Stracisz dostęp do platformy, szkoleń i materiałów</li>
          <li>Odblokowanie będzie możliwe tylko przez administratora</li>
          <li>Aby odblokować konto, napisz na: <a href="mailto:${supportEmail}" style="color:#D4A843;">${supportEmail}</a></li>
          <li>Lub skontaktuj się ze swoim Liderem</li>
        </ul>
      </div>
    ...
    await supabase.from("email_logs").insert({
      ...
      metadata: { 
        event: 'inactivity_final_warning',
    ...
    // Update inactivity_final_warning_sent_at
    await supabase
      .from("profiles")
      .update({ inactivity_final_warning_sent_at: new Date().toISOString() })
      .eq("user_id", userId);

    console.log("[send-inactivity-final-warning] Email sent successfully to:", profile.email);

    return new Response(
      JSON.stringify({ success: true, message: `Final inactivity warning sent to ${profile.email}` }),
      { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } }
    );
  } catch (error: any) {
    console.error("[send-inactivity-final-warning] Error:", error);
    return new Response(
