...
export const convertSupabaseSection = (section: any): CMSSection => ({
  ...
  // Normalize size types: if custom dimensions exist, ensure type is 'custom'
  width_type: (section.width_type === 'custom' || (section.custom_width && section.custom_width > 0)) ? 'custom' : (section.width_type || 'full'),
  height_type: (section.height_type === 'custom' || (section.custom_height && section.custom_height > 0)) ? 'custom' : (section.height_type || 'auto')
});
