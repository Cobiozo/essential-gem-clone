      ...
      try {
        ...
        await supabase.functions.invoke('send-webinar-confirmation', {
          body: {
            ...
            roomLink: isAutoWebinar && event?.slug ? `https://purelife.info.pl/a-w/${event.slug}${nextSlot ? `?slot=${nextSlot.time}` : ''}${email ? `&ref=${btoa(email)}` : ''}` : (isAutoWebinar ? `https://purelife.info.pl/auto-webinar` : undefined),
