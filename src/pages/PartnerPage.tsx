...
const PartnerPageView: React.FC = () => {
  ...
  useEffect(() => {
    const fetchPage = async () => {
      ...
      // Check if partner has a selected gallery template
      const selectedTemplateId = (pageData as any).selected_template_id;

      const [templateRes, galleryRes, profileRes, productsRes, linksRes] = await Promise.all([
        supabase.from('partner_page_template').select('template_data').limit(1).maybeSingle(),
        selectedTemplateId
          ? supabase.from('partner_page_templates_gallery' as any).select('template_data, template_type').eq('id', selectedTemplateId).eq('is_active', true).maybeSingle()
      ...
      // Priority: gallery template > legacy template
      const galleryData = galleryRes?.data as any;
      if (galleryData?.template_type === 'eqology_omega3') {
        setTemplateType('eqology_omega3');
        setEqologyData(galleryData.template_data as EqologyTemplateData);
      } else {
    ...
    <div className="min-h-screen bg-card">
      ...
      {heroElement?.content && (
        <section className="relative overflow-hidden bg-gradient-to-br from-primary/10 via-primary/5 to-background">
          <div className="absolute inset-0 opacity-[0.03]" style={{ backgroundImage: 'radial-gradient(circle at 25% 25%, hsl(var(--primary)) 1px, transparent 1px)', backgroundSize: '32px 32px' }} />
          ...
          <div className="max-w-6xl mx-auto px-4 sm:px-6 py-10 sm:py-14">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 sm:gap-6">
              {linkedProducts.map(({ product, purchase_url }) => (
                <a key={product!.id} href={purchase_url} target="_blank" rel="noopener noreferrer" className="group block bg-card border border-border rounded-2xl overflow-hidden shadow-sm hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
                  {product!.image_url && (
                    <div className="aspect-square overflow-hidden bg-muted">
                      <img src={product!.image_url} alt={product!.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                    </div>
                  )}
                  <div className="p-3 text-center"><h3 className="font-medium text-foreground text-sm">{product!.name}</h3></div>
                </a>
              ))}
            </div>
          </div>
              ...
              <div className="space-y-4">
                ...
                {contactEmail && (
                  <a href={`mailto:${contactEmail}`} className="flex items-center gap-3 text-sm text-muted-foreground hover:text-foreground transition-colors group">
                    <div className="w-9 h-9 rounded-lg bg-primary/10 flex items-center justify-center group-hover:bg-primary/20 transition-colors"><Mail className="w-4 h-4 text-primary" /></div>
                ...
                {contactPhone && (
                  <a href={`tel:${contactPhone}`} className="flex items-center gap-3 text-sm text-muted-foreground hover:text-foreground transition-colors group">
                    <div className="w-9 h-9 rounded-lg bg-primary/10 flex items-center justify-center group-hover:bg-primary/20 transition-colors"><Phone className="w-4 h-4 text-primary" /></div>
                    <span>{contactPhone}</span>
                  </a>
                )}
                {contactFacebook && (
                  <a href={contactFacebook.startsWith('http') ? contactFacebook : `https://${contactFacebook}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 text-sm text-muted-foreground hover:text-foreground transition-colors group">
                    <div className="w-9 h-9 rounded-lg bg-primary/10 flex items-center justify-center group-hover:bg-primary/20 transition-colors"><Facebook className="w-4 h-4 text-primary" /></div>
                    <span>Facebook</span>
                  </a>
                )}
              </div>
