...
export const useTeamContacts = () => {
  ...
  const fetchEventContactIds = useCallback(async () => {
    ...
    try {
      const { data, error } = await supabase
        .from('guest_event_registrations')
        .select('id, team_contact_id, event_id, first_name, email, status, registered_at, registration_attempts, slot_time, events(title, start_time), team_contacts!inner(id, deleted_at)')
