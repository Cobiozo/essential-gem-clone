...
export const ProductsGridSection: React.FC<Props> = ({ config, products, productLinks }) => {
  const { heading, columns } = config;

  // If template defines columns, render from config. Otherwise fall back to partner product links.
  const items = columns?.length > 0 ? columns : productLinks.map(lp => ({
    name: lp.product?.name || '',
    subtitle: '',
    specs: lp.product?.description || '',
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {(items || []).map((col: any, i: number) => {
            // Try to find partner purchase URL for this product
            ...
            return (
              <div key={i} className="bg-card rounded-2xl border border-border overflow-hidden shadow-sm flex flex-col">
                ...
                <div className="p-6 flex-1 flex flex-col">
                  ...
                  {col.subtitle && <p className="text-sm text-muted-foreground mb-3">{col.subtitle}</p>}
                  {col.specs && (
                    <Collapsible>
                  ...
                  <a
                    ...
                    className="mt-auto inline-flex items-center justify-center bg-primary text-primary-foreground px-6 py-3 rounded-xl font-semibold text-sm hover:opacity-90 transition-opacity"
                  >
                    {col.cta_text || 'KUP TERAZ'}
                  </a>
                </div>
              </div>
            );
