...
import type { ChallengeBannerConfig } from '@/hooks/useChallengeBanner';

interface Props {
  config: ChallengeBannerConfig;
  fallback?: React.ReactNode;
  embedded?: boolean;
}

export const ChallengeBanner: React.FC<Props> = ({ config, fallback, embedded = false }) => {
        ...
        <div className="flex items-center gap-3 mb-3 px-4 sm:px-0">
          ...
          <div>
            <h1 className="text-3xl md:text-4xl font-bold tracking-tight">
              {config.title || 'Wyzwanie 90-dniowe'}
            </h1>
            <p className="text-muted-foreground text-sm md:text-base">
              {config.subtitle || 'Codzienna dyscyplina. Realne efekty.'}
            </p>
          </div>
        </div>
      </Wrapper>
  ...
  return (
    <Wrapper>
      <div className="md:hidden">
        <img src={config.image_url} alt={config.title || 'Banner'} className="block w-full h-auto rounded-none sm:rounded-2xl shadow-xl" />
        {(config.title || config.subtitle || (config.cta_label && config.cta_url)) && (
          <div className={`flex flex-col gap-2 px-4 sm:px-0 py-4 ${alignClass}`}>
            {config.title && (
              <h1 className="font-bold tracking-tight leading-tight" style={{ color: config.title_color, fontSize: `${Math.min(config.title_size, 32)}px` }}>
                {config.title}
              </h1>
            )}
            {config.subtitle && (
              <p className="text-sm" style={{ color: config.subtitle_color }}>{config.subtitle}</p>
            )}
      ...
      <div className="relative w-full overflow-hidden rounded-2xl shadow-xl hidden md:block" style={{ height: `${config.height}px` }}>
        <div
        ...
        {config.overlay_gradient ? (
          ...
          <div className="absolute inset-0" style={{ backgroundColor: hexToRgba(config.overlay_color, config.overlay_opacity) }} />
        )}
        <div className={`relative h-full flex flex-col justify-end p-6 md:p-10 gap-3 ${alignClass}`}>
          {config.title && (
            <h1 className="font-bold tracking-tight leading-tight drop-shadow-lg" style={{ color: config.title_color, fontSize: `${config.title_size}px` }}>
              {config.title}
            </h1>
          )}
          {config.subtitle && (
