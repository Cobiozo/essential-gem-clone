...
export function VolumeBonusProgress({ clients, thresholds }: VolumeBonusProgressProps) {
  ...
  const formatClients = (value: number): string => {
    if (value >= 1000) {
      return `${(value / 1000).toFixed(0)} ${tf('calc.inf.thousand', 'tys.')}`;
    }
    ...
    <Card>
      <CardHeader className="pb-3">
        <CardTitle className="flex items-center gap-2 text-base">
          ...
          {tf('calc.inf.volumeBonuses', 'Bonusy Wolumenowe')}
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-3">
        {totalBonus > 0 && (
          <div className="text-center py-2 px-3 rounded-lg bg-emerald-50 dark:bg-emerald-950/30 mb-3">
            <p className="text-xs text-emerald-700 dark:text-emerald-400">{tf('calc.inf.totalBonuses', 'Suma bonusów')}</p>
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <span className={`text-sm ${isAchieved ? 'font-medium' : 'text-muted-foreground'}`}>
                    {formatClients(threshold.threshold_clients)} {tf('calc.spec.clients', 'klientów')}
                  </span>
                </div>
        ...
        {allAchieved && (
          <div className="text-center pt-2">
            <p className="text-xs text-emerald-600 font-medium">
              ✨ {tf('calc.inf.maxThreshold', 'Maksymalny próg osiągnięty!')}
            </p>
          </div>
        )}
      </CardContent>
