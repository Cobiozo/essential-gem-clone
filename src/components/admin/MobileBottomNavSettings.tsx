...
import { Trash2, Plus, ArrowUp, ArrowDown, Smartphone } from 'lucide-react';
    ...
    <Card>
      <CardHeader>
        ...
        <p className="text-sm text-muted-foreground">
          ...
          Ikony pochodzą z biblioteki <a href="https://lucide.dev/icons" target="_blank" rel="noreferrer" className="underline">lucide.dev/icons</a> — wpisz dokładną nazwę PascalCase (np. <code>Home</code>, <code>Calendar</code>, <code>MessageCircle</code>).
        </p>
      </CardHeader>
            ...
            <div key={it.id} className="border rounded-lg p-3 space-y-3 bg-card">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                ...
                <div>
                  ...
                  <Input defaultValue={it.icon_name} onBlur={(e) => e.target.value !== it.icon_name && update(it.id, { icon_name: e.target.value })} />
                </div>
                <div>
                  <Label className="text-xs">Ścieżka</Label>
                  <Input defaultValue={it.target_path} onBlur={(e) => e.target.value !== it.target_path && update(it.id, { target_path: e.target.value })} />
                </div>
              </div>
